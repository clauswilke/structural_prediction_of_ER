...
for(protein in levels(data$protein))
{
	...
	# r4s - omega:
		...
		x = cor.test( d$r4s, d$omega, method="spearman", na.action="na.omit" )
		r.omega = x$estimate
		p.omega = x$p.value
		
	# r4s - entropy:

		x = cor.test( d$r4s, d$entropy, method="spearman", na.action="na.omit" )
		r.entropy = x$estimate
		p.entropy = x$p.value

    row = data.frame( protein=protein, r.omega = r.omega, p.omega = p.omega, r.entropy = r.entropy, p.entropy = p.entropy)
    result = rbind( result, row )
    print( protein )
}
row.names(result) = c()
write.csv( result, "correlation_analysis/cor_tables/cor_r4s_entropy_omega.csv", row.names=F )
...
pdf( "correlation_analysis/figures/cor_r4s_entropy_omega.pdf", width=4.5, height=4, useDingbats=FALSE )
