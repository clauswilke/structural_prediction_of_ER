...
pca_data = data[,names(data) %in% c('protein','entropy','rsa_avg_md','wcn_avg_md','chi1_var_md','rmsf_avg_md','bfca','desent')] 
...
{	
	...
	# CALCULATE CORRELATIONS WITH ENTROPY
		...
		x = lm( entropy~PC1+PC2+PC3+PC4+PC5+PC6, data=rotated_pca_data )
		x2 = summary(x)
...
pdf( "correlation_analysis/figures/cor_entropy_PC.pdf", width=7, height=4, useDingbats=FALSE )
par( mai=c(0.65, 0.65, 0.0, 0.05), mgp=c(2, 0.5, 0), tck=-0.03 )
plot(0,xaxt='n',yaxt='n',bty='n',pch='',ylab=expression(paste("Entropy explained variance (", r^2, ")")),xlab='Principal Component (PC)', xlim=c(1,6.2),ylim=c(-.2,.25)) #, main = 'sequence entropy - contact number association', cex.main=0.8)
...
legend( 1.0, -.1, labels[1:3], pch=19, col=colors[1:3], bty='n', cex=0.9)
legend( 1.7, -.1, labels[4:6], pch=19, col=colors[4:6], bty='n', cex=0.9)
legend( 2.4, -.1, labels[7:9], pch=19, col=colors[7:9], bty='n', cex=0.9)
dev.off()