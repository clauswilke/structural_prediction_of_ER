# This function calculates the correlation between the two variables entropy and phi for each protein structure in the study and outputs the result into a CSV file. It also generates the corresponding graphs of entropy vs. dihedral angles correlations.

# Amir Shahmoradi, Saturday 9:23 PM, Jan 15, 2014, Wilke Lab, ICMB, UT Austin
...
for(protein in levels(data$protein))
{
    ...
    row = data.frame( protein=protein, phi_rho = r.phi_var_md, phi_P = p.phi_var_md, psi_rho = r.psi_var_md, psi_P = p.psi_var_md, chi1_rho = r.chi1_var_md, chi1_P = p.chi1_var_md )
    result = rbind( result, row )
    print( protein )
}
row.names(result) = c()
write.csv( result, "correlation_analysis/cor_tables/cor_entropy_dihedral.csv", row.names=F )


index = names(result) %in% c("phi_rho", "psi_rho", "chi1_rho") # columns we want to plot
...
pdf( "correlation_analysis/figures/cor_entropy_dihedral.pdf", width=4.5, height=4, useDingbats=FALSE )
par( mai=c(0.65, 0.65, 0.1, 0.05), mgp=c(2, 0.5, 0), tck=-0.03 )
plot(0,xaxt='n',yaxt='n',bty='n',pch='',ylab="Spearman's rank correlation coefficient",xlab='Dihedral angles: Phi, Psi, Chi1', xlim=c(1,3.2),ylim=c(-.5,.5)) #, main = 'sequence entropy - contact number association', cex.main=0.8)
#minor.tick(nx=0, ny=4, tick.ratio=2)
axis( 2 ) # y axis
axis( 1,
      at=c(1, 2, 3),
      c("variance Phi", "variance psi", "variance chi1"))
