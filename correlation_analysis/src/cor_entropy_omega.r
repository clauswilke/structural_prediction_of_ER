...
for(protein in levels(data$protein))
{
	...
	# ENTROPY - VARIABLES :
		...
		r.entropy_desent = x$estimate
		p.entropy_desent = x$p.value
	...
	# OMEGA - VARIABLES :
		...
		x = cor.test( d$desent, d$omega, method="spearman", na.action="na.omit" )
		r.omega_desent = x$estimate
		p.omega_desent = x$p.value
