...
for(protein in levels(data$protein))
{
    ...
    row = data.frame( protein=protein,
                      ...
                      avg_md_rho = r.rmsf_avg_md, avg_md_P = p.rmsf_avg_md, bfca_rho = r.bfca, bfca_P = p.bfca, avg_cr_rho = r.rmsf_cr, avg_cr_P = p.rmsf_cr ) # , std_md_rho = r.rmsf_std_md, std_md_P = p.rmsf_std_md
...
index = names(result) %in% c("phi_rho", "psi_rho", "chi1_rho", "avg_md_rho", "bfca_rho", "avg_cr_rho") # columns we want to plot
index.p = names(result) %in% c("phi_P", "psi_P", "chi1_P", "avg_cr_P", "avg_md_P", "bfca_P") # columns that store significance
...
pdf( "correlation_analysis/figures/cor_entropy_SF.pdf", width=9, height=4, useDingbats=FALSE )
par( mai=c(0.65, 0.65, 0.1, 0.05), mgp=c(2, 0.5, 0), tck=-0.03 )
plot(0,xaxt='n',yaxt='n',bty='n',pch='',ylab=expression(paste("Correlation (", rho, ") with entropy")),xlab='Correlating Variable', xlim=c(1,6.2),ylim=c(-.5,.5)) #, main = 'sequence entropy - contact number association', cex.main=0.8)
...
axis( 1,
      at=c(1, 2, 3, 4, 5, 6),
      c(expression(paste("variance ", phi)), expression(paste("variance ", psi)), expression(paste("variance ", chi[1])), "RMSF (MD)", expression(paste("B-factor @ C", alpha)), "RMSF (CS)"))
