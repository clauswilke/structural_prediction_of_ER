# This function calculates the correlation between the two variables rsa_cr and the b-factors (bfca) of the CA atoms of the protein backbone for each protein structure in the study and outputs the result into a CSV file. It also generates the corresponding graphs of rsa_cr vs. bfca correlations?

# Amir Shahmoradi, Monday 2:17 PM, Feb 3 2014, Wilke Lab, ICMB, UT Austin
...
for(protein in levels(data$protein))
{
	...
	x = cor.test( d$bfca, d$rsa_cr, method="spearman", na.action="na.omit" )
    r.bfca = x$estimate
    p.bfca = x$p.value
...
row.names(result) = c()
write.csv( result, "correlation_analysis/cor_tables/cor_rsa_cr_bfca.csv", row.names=F )
...
#
#pdf( "correlation_analysis/figures/cor_rsa_cr_bfca.pdf", width=4.5, height=4, useDingbats=FALSE )
#par( mai=c(0.65, 0.65, 0.1, 0.05), mgp=c(2, 0.5, 0), tck=-0.03 )
#plot(0,xaxt='n',yaxt='n',bty='n',pch='',ylab="Spearman's rank correlation coefficient",xlab='Weighted Contact Number: bfca', xlim=c(1,3.2),ylim=c(-.5,.5)) #, main = 'sequence rsa_cr - contact number association', cex.main=0.8)
