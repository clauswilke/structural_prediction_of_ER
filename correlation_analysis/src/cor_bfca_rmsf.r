...
pdf( "correlation_analysis/figures/cor_bfca_rmsf.pdf", width=9, height=8, useDingbats=FALSE )

split.screen(c(1,2))

screen(1)
	par( mai=c(0.65, 0.65, 0.1, 0.05), mgp=c(2, 0.5, 0), tck=-0.03 )
	plot(0,xaxt='n',yaxt='n',bty='n',pch='',xlab="B factor",ylab="RMSF (MD simulation)", xlim=c(-.4,.4),ylim=c(-.4,.4))
	axis( 1,  # x axis
	...
	points( result$r.bfca_rmsfHS, result$r.rsa_avg_md, pch=19 )
	abline(0,1)
