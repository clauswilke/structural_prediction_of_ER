...
data = rbind(data_1RD8_AB, data_2FP7_B, data_2Z83_A, data_3LYF_A, data_4AQF_B, data_4GHA_A, data_4IRY_A) #, data_3GSZ_A, data_3I5K_A, data_2JLY_A_temp_50, data_2JLY_A_temp_100, data_2JLY_A_temp_200, data_2JLY_A_temp_450)
...
for(protein in levels(data$protein))
{
	...
	x = cor.test( d$rsa_avg_md, d$r4s, method="spearman", na.action="na.omit" )
    r.rsa_avg_md = x$estimate
    p.rsa_avg_md = x$p.value
    
	x = cor.test( d$wcn_avg_md, d$r4s, method="spearman", na.action="na.omit" )
    r.wcn_avg_md = x$estimate
    p.wcn_avg_md = x$p.value

	x = cor.test( d$chi1_var_md, d$r4s, method="spearman", na.action="na.omit" )
    r.chi1_var_md = x$estimate
    p.chi1_var_md = x$p.value
	
	x = cor.test( d$rmsf_avg_md, d$r4s, method="spearman", na.action="na.omit" )
    r.rmsf_avg_md = x$estimate
    p.rmsf_avg_md = x$p.value
	
	x = cor.test( d$bfca, d$r4s, method="spearman", na.action="na.omit" )
    r.bfca = x$estimate
    p.bfca = x$p.value
	
	#x = cor.test( d$rmsf_std_md, d$r4s, method="spearman", na.action="na.omit" )
    #r.rmsf_std_md = x$estimate
    #p.rmsf_std_md = x$p.value

#	if (length(na.omit(d$rmsfHS))>=3)
#	{
#		x = cor.test( d$rmsfHS, d$r4s, method="spearman", na.action="na.omit" )
...
#	}
    ...
    row = data.frame( protein=protein,
                      ...
                      r.bfca = r.bfca, p.bfca = p.bfca) # avg_cr_rho = r.rmsf_cr, avg_cr_P = p.rmsf_cr ) # , std_md_rho = r.rmsf_std_md, std_md_P = p.rmsf_std_md
    result = rbind( result, row )
    print( protein )
}
row.names(result) = c()
write.csv( result, "correlation_analysis/cor_tables/cor_r4s_all.csv", row.names=F )
