...
for(protein in levels(data$protein))
{
	...
	#x = cor.test( d$rmsf_std_md, d$omega, method="spearman", na.action="na.omit" )
...
row.names(result) = c()
write.csv( result, "correlation_analysis/cor_tables/cor_omega_all.csv", row.names=F )
...
pdf( "correlation_analysis/figures/cor_omega_all.pdf", width=7, height=4, useDingbats=FALSE )
par( mai=c(0.65, 0.65, 0.0, 0.05), mgp=c(2, 0.5, 0), tck=-0.03 )
plot(0,xaxt='n',yaxt='n',bty='n',pch='',ylab=expression(paste("Correlation (", rho, ") with omega")),xlab='Correlating Variable', xlim=c(1,5.2),ylim=c(-.5,.45)) #, main = 'sequence omega - contact number association', cex.main=0.8)
