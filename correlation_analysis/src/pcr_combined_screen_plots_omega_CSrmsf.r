...
out_path = 'C:/Users/Amir/Documents/GitHub/structural_prediction_of_ER/correlation_analysis/pcr_combined/pcr_combined_cors_omega_CSrmsf.csv'
...
pca_data = data[,names(data) %in% c('protein','omega','desent','rsa_avg_md','rmsf_avg_md','chi1_var_md','wcn_avg_md','bfca', 'rmsfHS')] 
...
{	
	...
	# CALCULATE CORRELATIONS WITH omega
		...
		x = cor.test( rotated_pca_data$omega, rotated_pca_data$PC7 )
		r.PC7 = x$estimate
		p.PC7 = x$p.value
...
pdf( "correlation_analysis/figures/PC_screen_omega_CSrmsf.pdf", width=9.3, height=9.2, useDingbats=FALSE )
...
screen(1)
	...
	plot(0,xaxt='n',yaxt='n',bty='n',pch='',ylab=expression(paste(omega," explained variance (", r^2, ")")),xlab='Principal Component (PC)', xlim=c(1,7.2),ylim=c(-.1,.2))
