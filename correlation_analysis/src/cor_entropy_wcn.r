# This function calculates the correlation between the two variables entropy and the Weighted Contact Number (WCN) for each protein structure in the study and outputs the result into a CSV file. It also generates the corresponding graphs of entropy vs. WCN correlations.

# Amir Shahmoradi, Saturday 1:46 AM, Jan 25, 2014, Wilke Lab, ICMB, UT Austin
...
for(protein in levels(data$protein))
{
    ...
    row = data.frame( protein=protein, cr_rho = -r.wcn_cr, cr_P = p.wcn_cr, avg_md_rho = -r.wcn_avg_md, avg_md_P = p.wcn_avg_md, var_md_rho = -r.wcn_var_md, var_md_P = p.wcn_var_md )
    result = rbind( result, row )
    print( protein )
}
row.names(result) = c()
write.csv( result, "correlation_analysis/cor_tables/cor_entropy_wcn.csv", row.names=F )
...
pdf( "correlation_analysis/figures/cor_entropy_wcn.pdf", width=4.5, height=4, useDingbats=FALSE )
par( mai=c(0.65, 0.65, 0.1, 0.05), mgp=c(2, 0.5, 0), tck=-0.03 )
plot(0,xaxt='n',yaxt='n',bty='n',pch='',ylab="Spearman's rank correlation coefficient",xlab='Weighted Contact Number: WCN', xlim=c(1,3.2),ylim=c(-.5,.5)) #, main = 'sequence entropy - contact number association', cex.main=0.8)
