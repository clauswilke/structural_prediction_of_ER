# This R code performs Principal Component ANALYSIS on different variables of THE COMBINED PROTEINS' DATA potentially correlating with entropy, then finds the rotation matrix, then outputs the component loadings and the percentage of the entropy's explained variance by each principal component in a CVS file.
# ATTN: In this version, only structures are considered for which RMSF from crystal structures is available

# Amir Shahmoradi, Friday 5:35 PM, March 28 2014, Wilke Lab, ICMB, UT Austin

# source("input_data.r")

out_path = 'C:/Users/Amir/Documents/GitHub/structural_prediction_of_ER/correlation_analysis/pcr_combined/pcr_combined_cors_entropy_CSrmsf.csv'
...
pca_data = subset(pca_data, rmsfHS != 'NA')		# Exclude all 'rmsfHS = NA' rows in pca_data
	...
	pca_data$rmsfHS      = scale(pca_data$rmsfHS      , center = TRUE, scale = TRUE)
	...
	#	names(pca_data) <- c('protein','entropy','designed_entropy','RSA','RMSF','VAR(chi1)','iWCN','Bfactor')
...
{	
	...
	# CALCULATE CORRELATIONS WITH ENTROPY
		
		x = cor.test( rotated_pca_data$entropy, rotated_pca_data$PC1 )
		r.PC1 = x$estimate
		p.PC1 = x$p.value

		x = cor.test( rotated_pca_data$entropy, rotated_pca_data$PC2 )
		r.PC2 = x$estimate
		p.PC2 = x$p.value

		x = cor.test( rotated_pca_data$entropy, rotated_pca_data$PC3 )
		r.PC3 = x$estimate
		p.PC3 = x$p.value

		x = cor.test( rotated_pca_data$entropy, rotated_pca_data$PC4 )
		r.PC4 = x$estimate
		p.PC4 = x$p.value
		
		x = cor.test( rotated_pca_data$entropy, rotated_pca_data$PC5 )
		r.PC5 = x$estimate
		p.PC5 = x$p.value
		
		x = cor.test( rotated_pca_data$entropy, rotated_pca_data$PC6 )
		r.PC6 = x$estimate
		p.PC6 = x$p.value

		x = cor.test( rotated_pca_data$entropy, rotated_pca_data$PC7 )
		r.PC7 = x$estimate
		p.PC7 = x$p.value
		
		x = lm( entropy~PC1+PC2+PC3+PC4+PC5+PC6+PC7, data=rotated_pca_data )
		x2 = summary(x)
	...
	# Combine correlation values in row for each protein and dump it in RESULT data.frame
		...
		row = data.frame( protein=protein,
						  ...
						  rsq.PC7 = r.PC7^2, p.PC7 = p.PC7, 
						  rsq.total = x2$adj.r.squared )
		sum = sum(unlist(row[1,c(2,4,6,8,10,12,14)]))
...
screen(1)
	...
	plot(0,xaxt='n',yaxt='n',bty='n',pch='',ylab=expression(paste("Entropy explained variance (", r^2, ")")),xlab='Principal Component (PC)', xlim=c(1,7.2),ylim=c(-.1,.2)) #, main = 'sequence entropy - contact number association', cex.main=0.8)
