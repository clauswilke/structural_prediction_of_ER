...
#initialize matrix for seq-s with difference
final_aln <- matrix(" 0", nrow = 1 , ncol = length(refseq))
final_aln[1,] = refseq #first sequence used for comparison is reference seq

#creating vector with sorted pdb names
pdbid = c()
pdbid[1] = refpdb

#sort through sequences to find unique ones
for (i in 1:length(pdbs$ali[,1])){
        if (length(pdbid) > 1) {
                ...
                #find all places where sequence in the original aln and refseq differ 
...
#realign found non unique sequences
