...
\author{Amir Shahmoradi,$^{1,2}$ Dariya K. Sydykova,$^{2}$ Stephanie J. Spielman,$^{2}$ \\
...
$^2$ Department of Integrative Biology, Center for Computational Biology and Bioinformatics, and Institute for Cellular and Molecular Biology, The University of Texas at Austin, TX, 78712 \\ }
...
Patterns of sequence variation in protein-coding genes are shaped by the biophysical structure and function of the expressed proteins
...
\citep{Overingtonetal1992,Goldmanetal1998,MirnyShakhnovich1999,Deanetal2002}. More specifically, when evolutionary variation is plotted as a function of Relative Solvent Accessibility (RSA, a measure of residue buriedness), it falls, on average, onto a straight line \citep{FranzosaXia2009,Ramseyetal2011,FranzosaXia2012,Scherreretal2012}. We need to emphasize, however, that this relationship represents an average over many sites and many proteins. At the level of individual sites in individual proteins, RSA is often only weakly correlated with evolutionary variation \citep{MeyerWilke2013,Meyeretal2013,Yehetal2014}.

Measures of residue buriedness other than RSA have also been shown to correlate with sequence variability, for example residue contact number \citep{Liaoetal2005,FranzosaXia2009,Yehetal2014}. Some have argued that contact number serves as a better predictor of evolutionary variation than RSA \citep{Yehetal2014}. Because contact number may be a proxy of residue and site-specific backbone flexibility \citep{Halle2002}, one may also expect to observe a positive trend between local structural flexibility and sequence variability \citep{Yehetal2014}. Indeed, several authors have argued for the potential role of protein dynamics in sequence variability \citep{LiuBahar2012,NevinGereketal2013,MarshTeichmann2014}. However, a recent paper argued against the flexibility model, on the grounds that it should introduce certain non-linearities that are not observed in the data \citep{Huangetal2014}.

Measures of residue buriedness or packing density can be calculated in a straightforward manner from individual crystal structures. By contrast, measures of structural flexibility, either at the side-chain or the backbone level, are more difficult to obtain. Ideally, one would want to observe physiologically relevant structural fluctuations \emph{in vivo}. An approximate measure of such fluctuations could be the variations observed in NMR ensembles. Further, the thermal motion of atoms in a crystal are recorded in B factors, which are available for every atom in every PDB structure. However, it is unclear how accurately B factors reflect structural fluctuations of a protein in solution. One can also attempt to assess physiologic fluctuations by aligning multiple homologous PDB structures. Alternatively, one can consider a simulation approach, either using coarse-grained modeling, e.g. via Elastic Network Models \citep{Sanejouand2013}, or using atom-level modeling, e.g. via molecular dynamics (MD) \citep{KarplusMcCammon2002}. It is currently not well understood which (if any) of these measures of structural fluctuations are predictive of evolutionary variation, and to what extent they provide distinct insight into the evolutionary process.

Here, we provide a comprehensive analysis of the extent to which numerous different structural quantities predict evolutionary variation. As structural predictors, we include both measures of buriedness, such as RSA and contact number, and measures of structural fluctuations, including B factors, several measures of backbone and side-chain variability obtained from MD simulations, and backbone variability obtained from alignments of homologous crystal structures. We also include site variability predicted from computational protein design. On a set of 9 viral proteins, we find that measures of buriedness generally perform better at predicting evolutionary site variation than either measures of structural fluctuations or computational protein design. Among the measures of structural fluctuations, measures of side-chain variability seem to perform better than measures of backbone variability, possibly because the former are more closely correlated with residue buriedness. Finally, site variability predicted from computational protein design performs worse than the best-performing measures of structural fluctuations.
...
\noindent Figure S1: \textbf{Principal Component (PC) Regression of sequence entropy against the structural variables, including CS RMSF.} {\bf (A)} Variance in entropy explained by each principal component. For most proteins, PC1 and either PC2 or PC3 show the strongest correlations with sequence entropy. Significant correlations ($P<0.05$) are shown as filled symbols, and insignificant correlations ($P\leq0.05$) are shown as open symbols. {\bf (B)} and {\bf (C)} Composition of the three leading components. Red arrows represent the loadings of each of the structural variables on the principal components; black dots represent the amino acid sites in the PC coordinate system.
\customlabel{fig:cor_entropy_PC_screen_CSrmsf}{S1}


\bigskip
\centerline{\includegraphics[width=6.5in]{PC_screen_omega.pdf}}
\noindent Figure S2: \textbf{Principal Component (PC) Regression of $\omega$ against the structural variables.} {\bf (A)} Variance in $\omega$ explained by each principal component. For most proteins, PC1 and PC3 show the strongest correlations with $\omega$. Significant correlations ($P<0.05$) are shown as filled symbols, and insignificant correlations ($P\leq0.05$) are shown as open symbols. {\bf (B)} and {\bf (C)} Composition of the three leading components. Red arrows represent the loadings of each of the structural variables on the principal components; black dots represent the amino acid sites in the PC coordinate system. Note that parts B and C are identical to those shown in Figure~\ref{fig:cor_entropy_PC_screen}.
\customlabel{fig:cor_omega_PC_screen}{S2}


\bigskip
\centerline{\includegraphics[width=6.5in]{PC_screen_omega_CSrmsf.pdf}}
\noindent Figure S3: \textbf{Principal Component (PC) Regression of $\omega$ against the structural variables, including CS RMSF.} {\bf (A)} Variance in $\omega$ explained by each principal component. For most proteins, PC1 and either PC2 or PC3 show the strongest correlations with $\omega$. Significant correlations ($P<0.05$) are shown as filled symbols, and insignificant correlations ($P\leq0.05$) are shown as open symbols. {\bf (B)} and {\bf (C)} Composition of the three leading components. Red arrows represent the loadings of each of the structural variables on the principal components; black dots represent the amino acid sites in the PC coordinate system. Note that parts B and C are identical to those shown in Figure~\ref{fig:cor_entropy_PC_screen_CSrmsf}.
